...
const ArticleHelper = {
  ...
  updateValidate: (payload) => {
    const schema = Joi.object({
      ...
      article_type: Joi.number().valid(0, 1),

      best_film_critic_id: Joi.number().required(),

      writer_name: Joi.string(),

      article_title: Joi.string(),

      language_id: Joi.array().items(Joi.number()).default([]),

      other_language: Joi.alternatives().conditional("language_id", {
        is: Joi.array()
          .items(Joi.number())
          .custom((value, helpers) => {
            if (value && value.includes(24)) return value;
            return helpers.error("not.includes.24");
          }),
        then: Joi.string().required().messages({
          "any.required":
            "Other language is required when language_id includes 24.",
      ...
      date_of_publication: Joi.date().iso(),

      name_of_publication: Joi.string(),

      publisher_name: Joi.string(),

      publisher_email: Joi.string().email(),

      publisher_mobile: Joi.string(),

      publisher_landline: Joi.string().allow(null, ""),

      publisher_address: Joi.string(),

      publisher_citizenship: Joi.string(),
